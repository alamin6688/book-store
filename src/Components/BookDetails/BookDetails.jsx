...
const BookDetails = () => {
  ...
  const handleReadButtonClick = () => {
    if (!readList.includes(selectedBook?.bookId)) {
      ...
      localStorage.setItem(
        "readList",
        JSON.stringify([...readList, selectedBook.bookId])
      );
  ...
  const handleWhitelistButtonClick = () => {
    if (!wishlist.includes(selectedBook?.bookId)) {
      if (readList.includes(selectedBook?.bookId)) {
        toast.error(
          "Book already added to Read list, can't be added to Wishlist!"
        );
      } else {
        ...
        localStorage.setItem(
          "wishlist",
          JSON.stringify([...wishlist, selectedBook.bookId])
        );
          ...
          <div className="flex-1 space-y-3">
            ...
            <h1>
              {selectedBook.tags.map((tag, index) => (
                <li className="list-none inline mr-5" key={index}>
                  {tag}
                </li>
              ))}
            </h1>
            ...
            <div className="flex gap-5">
              <button
                ...
                className="btn hover:bg-green-600 hover:text-white text-black border bg-transparent border-gray-400 px-6"
              >
                Read
              </button>
              <button
                onClick={handleWhitelistButtonClick}
                className="btn bg-blue-400 hover:bg-blue-600  border-none text-white"
              >
                Wishlist
              </button>
            </div>
