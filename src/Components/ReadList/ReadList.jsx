...
const ReadList = () => {
  ...
  useEffect(() => {
    ...
    const storedBookList = storedReadList.map((bookId) =>
      ...
      <div className="listed-books">
        {bookList.length > 0 ? (
          bookList.map((book) => (
            <div
              key={book.bookId}
              className="border flex flex-col lg:flex-row my-10 gap-5 p-5 rounded-xl items-center"
            >
              <img
                src={book.image}
                className="w-[26%] rounded-xl mx-auto lg:mx-0"
              />
              <div className="space-y-2">
                ...
                <div className="mt-2 flex items-center">
                  ...
                  {book.tags.map((tag, index) => (
                    <button
                      key={index}
                      className="bg-[#F4FCF3] text-[#57CD45] px-3 py-1 rounded-full mr-2"
                    >
                      {tag}
                    </button>
                  ))}
                ...
                <div className="flex justify-between">
                  ...
                  <div className="flex items-center">
                    <LuFiles />
                    <h1 className="ml-2 text-lg mr-2">
                ...
                <div className="flex flex-col md:flex-row gap-5">
                  <button className="btn border-none rounded-full text-[#328EFF] bg-[#E0EEFF] btn-primary">
                    Category: {book.category}
                  </button>
                  <button className="btn border-none rounded-full text-[#FFAC33] bg-[#FFF3E0]  btn-primary">
                    Rating: {book.rating}
                  </button>
